...
parser.add_argument("--pfam_db", help="The pfam Database file formated for Hmmer", type =str, default="/mnt/lustre/repos/bio/databases/public/pfam/pfam_release_34.0/Pfam-A.hmm")
parser.add_argument("--pfam_hits_file", help="The pfam pairwise table generated by Virathon", type =str)
...
parser.add_argument("--kegg_db", help="The KEGG Database (KOfam) file formated for Hmmer", type =str, default="/mnt/lustre/bio/users/fcoutinho/KOfam/All_KOs.hmm")
...
parser.add_argument("--uniref_db", help="The UniRef100 Database formated for DIAMOND", type =str, default="/mnt/lustre/repos/bio/databases/public/UniProtKB/UniProt_Release_2021-04-07/uniref100.dmnd")
parser.add_argument("--uniref_hits_file", help="The Uniref blastp file generated by Diamond", type =str, default="NA")
parser.add_argument("--enriched_uniref_hits_file", help="The Uniref Enriched blastp file generated by Diamond + Enrich_m8", type =str)
...
parser.add_argument("--ko_hmm_dir", help="Directory where KEGG KO hmm models are located", default="/mnt/lustre/bio/users/fcoutinho/KOfam/profiles/", type=str)
...
def central():
    if (args.annotate == True):
        ...
        if (args.skip_uniref == False):
            ...
            parse_uniref(uniref_hits_file=enriched_uniref_hits_file,min_score=args.uniref_min_score,max_evalue=args.uniref_max_evalue)
...
def parse_hmmer_output(hmmer_out_file="NA",max_evalue=0.00001,min_score=50,multi_hits=False,prefix="NA",output_df_file="Info.tsv",print_df=True):
    ...
    for cds in cds_seq_info['Length']:
        ...
        if (prefix == "KEGG"):
            ...
            #When dealing with KEGG hits, get the KEGG info for the best hit of the cds if it was assigned to a KO. Note that KEGG_Best_Subject_Function hols a single function associated with the best hit of the CDS, while KEGG_Matched_Functions holds the set of the functons associated with all the KOS matched to the scaffold.
            if (ko != "NA"):
                    ...
                    for h1_name in kegg_info[ko][categ].keys():
                        for h2_name in kegg_info[ko][categ][h1_name].keys():
                            for h3_name in kegg_info[ko][categ][h1_name][h2_name].keys():
                                ...
                                genome_seq_info['KEGG_Matched_Functions'][scaffold_id].update(cds_seq_info['KEGG_Best_Subject_Function'][cds])
                                ...
                                genome_seq_info['KEGG_Matched_Subject_IDs'][scaffold_id].update(cds_seq_info['KEGG_Best_Subject'][cds])
        #Get the best subject ID for the cds Pfam domain hits
        if ((prefix == "Pfam") and (cds in cds_seq_info['Pfam_Subjects'])):
            genome_seq_info['Pfam_Subjects'][scaffold_id].update(cds_seq_info['Pfam_Subjects'])
...
def index_seqs(cds_file="NA"):
    ...
    for seqobj in SeqIO.parse(cds_file, "fasta"):
        ...
        if (scaffold_id not in seen_scaff_ids):
            ...
            genome_seq_info['Pfam_Subjects'][scaffold_id] = set()
...
def parse_uniref(uniref_hits_file="NA",min_score=50,max_evalue=0.00001):
