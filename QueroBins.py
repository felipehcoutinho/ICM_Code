...
def central():
    ...
    if (args.make_bins):
        (seq_info,bin_info) = binning_module(assemblies_files_list = assemblies_list, threads = args.threads, method = args.binning_method, min_length = args.min_scaffold_length, seq_info = seq_info)
    if (args.call_checkm):
...
def annotation_module(assemblies_list = "NA", seq_info = "NA"):
    ...
    for assembly_file in assemblies_list:
        ...
        if (args.parse_only == False):
            ...
            subprocess.call(command, shell=True)
        if (args.parse_only == False):
            command = f"CAT add_names -i out.CAT.contig2classification.txt -o Taxonomy_out.CAT.contig2classification.txt -t {args.cat_taxonomy}"
            print(f"Running: {command}")
            subprocess.call(command, shell=True)
        print("Processing CAT output file")
        with open("Taxonomy_out.CAT.contig2classification.txt","r") as INPUT:
...
def binning_module(seq_info = "NA", min_length = 2000, assemblies_files_list = "NA", threads = 1, method = "metabat"):
    if (method == "metabat"):
